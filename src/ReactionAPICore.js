...
const coreGraphQLSubscriptionSchema = importAsString(
...
export default class ReactionAPICore {
    ...
    if (functionsByType) {
      Object.keys(functionsByType).forEach((type) => {
        ...
        functionsByType[type].forEach((func) => {
          const entryWithSameName = this.functionsByType[type].find(
            (existingEntry) => existingEntry.func.name === func.name
          );
          ...
          for (const collectionKey in pluginConfig.collections) {
            if (
              ...
              if (
                ...
                throw new Error(
                  `In registerPlugin, collection "${collectionKey}" needs a name property`
                );
              }
              ...
              if (Array.isArray(collectionConfig.indexes)) {
                const indexingPromises = collectionConfig.indexes.map(
                  (indexArgs) =>
                    collectionIndex(
  ...
  async connectToMongo(options = {}) {
    ...
    // console.log("mongo client is ", client);

    this.mongoClient = client;

    // console.log("this.mongo client is ", this.mongoClient);

    await this.setMongoDatabase(client.db()); // Uses db name from the connection string

    this.context["mongoDbClientUpdated"] = client;
  }
  ...
  async runServiceStartup() {
    ...
    const registerPluginHandlerFuncs = this.context.getFunctionsOfType(
    ...
    registerPluginHandlerFuncs.forEach((registerPluginHandlerFunc) => {
      if (typeof registerPluginHandlerFunc !== "function") {
        throw new Error(
          'A plugin registered a function of type "registerPluginHandler" which is not actually a function'
        );
      }
    ...
    if (migrationsControl && migrationsControl.version < 76) {
      throw new Error(
        `Detected a migration version (${migrationsControl.version}) for the previous migration system, which is less than 76.` +
    ...
    if (Array.isArray(preStartupFunctionsRegisteredByPlugins)) {
      ...
      for (const preStartupFunctionInfo of preStartupFunctionsRegisteredByPlugins) {
        Logger.info(
          `Running pre-startup function "${preStartupFunctionInfo.func.name}" for plugin "${preStartupFunctionInfo.pluginName}"...`
        );
        const startTime = Date.now();
        await preStartupFunctionInfo.func(this.context); // eslint-disable-line no-await-in-loop
        const elapsedMs = Date.now() - startTime;
        Logger.info(
          `pre-startup function "${preStartupFunctionInfo.func.name}" for plugin "${preStartupFunctionInfo.pluginName}" finished in ${elapsedMs}ms`
        );
      }
    }
    ...
    if (Array.isArray(startupFunctionsRegisteredByPlugins)) {
      ...
      for (const startupFunctionInfo of startupFunctionsRegisteredByPlugins) {
        Logger.info(
          `Running startup function "${startupFunctionInfo.func.name}" for plugin "${startupFunctionInfo.pluginName}"...`
        );
        const startTime = Date.now();
        await startupFunctionInfo.func(this.context); // eslint-disable-line no-await-in-loop
        const elapsedMs = Date.now() - startTime;
        Logger.info(
          `Startup function "${startupFunctionInfo.func.name}" for plugin "${startupFunctionInfo.pluginName}" finished in ${elapsedMs}ms`
        );
      }
    }
  }
  ...
  initServer() {
    ...
    if (
      ...
      throw new Error(
        "Subscriptions are not supported with Apollo Federation. Set `REACTION_GRAPHQL_SUBSCRIPTIONS_ENABLED=false` to disable subscriptions."
      );
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        this.httpServer.listen({ port }, () => {
          ...
          if (!silent) {
            Logger.info(
              `GraphQL listening at ${this.graphQLServerUrl} (port ${
                port || "unknown"
              })`
            );
            ...
            if (this.hasSubscriptionsEnabled) {
              Logger.info(
                `GraphQL subscriptions ready at ${
  ...
  async retryStartServer(port, silent) {
    Logger.error(
      `Port ${port} is in use. Stop whatever is listening on that port. Retrying in 5 seconds...`
    );
    setTimeout(() => {
  ...
  async start(options = {}) {
    ...
    if (shouldInitReplicaSet) {
      try {
        ...
        Logger.warn(
          `Failed to initialize a MongoDB replica set. This may result in errors or some things not working. Error: ${error.message}`
        );
      }
    }
  ...
  async stop() {
    ...
    if (Array.isArray(shutdownFunctionsRegisteredByPlugins)) {
      ...
      for (const shutdownFunctionInfo of shutdownFunctionsRegisteredByPlugins) {
        Logger.info(
          `Running shutdown function "${shutdownFunctionInfo.func.name}" for plugin "${shutdownFunctionInfo.pluginName}"...`
        );
        const startTime = Date.now();
        await shutdownFunctionInfo.func(this.context); // eslint-disable-line no-await-in-loop
        const elapsedMs = Date.now() - startTime;
        Logger.info(
          `Shutdown function "${shutdownFunctionInfo.func.name}" for plugin "${shutdownFunctionInfo.pluginName}" finished in ${elapsedMs}ms`
        );
      }
    }
  ...
  async registerPlugin(plugin = {}) {
    if (typeof plugin.name !== "string" || plugin.name.length === 0) {
      throw new Error(
        "Plugin configuration passed to registerPlugin must have 'name' field"
      );
    }

    if (this.registeredPlugins[plugin.name]) {
      throw new Error(
        `You registered multiple plugins with the name "${plugin.name}"`
      );
    }
    ...
    if (Array.isArray(plugin.expressMiddleware)) {
      this.expressMiddleware.push(
        ...plugin.expressMiddleware.map((def) => ({
          ...def,
    ...
    if (plugin.contextAdditions) {
      Object.keys(plugin.contextAdditions).forEach((key) => {
        if ({}.hasOwnProperty.call(this.context, key)) {
          throw new Error(
            `Plugin ${plugin.name} is trying to add ${key} key to context but it's already there`
          );
        }
    ...
    Logger.info(
      `Registered plugin ${plugin.name} (${plugin.version || "no version"})`
    );
  }
